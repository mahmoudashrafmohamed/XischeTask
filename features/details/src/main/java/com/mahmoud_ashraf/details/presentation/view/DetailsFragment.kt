...
class DetailsFragment : Fragment() {
  ...
  private val university by lazy {  arguments?.getParcelable<GetUniversitiesResponseItem>(UNIVERSITIES_ITEM)}
  ...
  private fun initView(university: GetUniversitiesResponseItem) {
    with(binding){
      tvName.text = university.name
      tvState.text = university.stateProvince
      tvCountry.text = university.country
      tvWebpage.text = university.webPages?.firstOrNull()
